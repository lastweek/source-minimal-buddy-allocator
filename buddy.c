...
{
	return buddy_zone->page_map + pfn;
}

static inline unsigned long page_to_pfn(struct page *page)
{
	return page - buddy_zone->page_map;
}
...
static void
__free_one_page(struct buddy_zone *zone,
...
free_one_page(struct buddy_zone *zone,
	      struct page *page, unsigned long pfn, unsigned int order)
{
	/*
	 * Sanity check.
...
static inline void
expand(struct buddy_zone *zone, struct page *page,
...
__rmqueue(struct buddy_zone *zone, unsigned int order)
{
...
static __always_inline struct page *
rmqueue(struct buddy_zone *zone, unsigned int order)
{
	struct page *page;
...
int main(void)
{
	 ...
	 */
	nr_pages = PHYS_PFN(buddy_mem_end);
	buddy_zone->page_map = malloc(nr_pages * sizeof(struct page));
	if (!buddy_zone->page_map) {
		printf("Fail to alloc page_map. nr_pages: %d\n", nr_pages);
		free(zone);
		return -ENOMEM;
	}

	initial_free_pages(buddy_mem_start, buddy_mem_end);

	dprintf_INFO("Buddy managing [%#018lx-%#018lx] Page Size %lu KB\n",
		buddy_mem_start, buddy_mem_end,
		PAGE_SIZE >> 10);
	dump_buddy();
